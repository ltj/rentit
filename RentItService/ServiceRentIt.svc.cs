...
namespace RentItService {
    ...
    public class ServiceRentIt : IRentIt {
        ...
        public BookInfo GetBookInfo(int id) {
            ...
            RentItService.MediaRating mediaRating = this.CollectMediaReviews(book.media_id, rating, db);

            return RentItService.BookInfo.ValueOf(book, mediaRating);
        }
        ...
        public MovieInfo GetMovieInfo(int id) {
            ...
            RentItService.MediaRating mediaRating = this.CollectMediaReviews(movie.media_id, rating, db);

            return RentItService.MovieInfo.ValueOf(movie, mediaRating);
        }
        ...
        public AlbumInfo GetAlbumInfo(int id) {
            ...
            RentItService.MediaRating mediaRating = this.CollectMediaReviews(album.media_id, rating, db);
            ...
            List<RentItService.SongInfo> albumSongs = new List<SongInfo>();
            foreach (RentItDatabase.Song song in songs) {
                RentItDatabase.Rating songRatings = this.GetMediaRating(album.media_id, db);
                RentItService.MediaRating songRating = this.CollectMediaReviews(song.media_id, songRatings, db);

                albumSongs.Add(SongInfo.ValueOf(song, songRating));
            }

            return RentItService.AlbumInfo.ValueOf(album, albumSongs, mediaRating);
        }
        ...
        /// media item.
        /// </returns>
        private RentItService.MediaRating CollectMediaReviews(
            int mediaId,
            ...
            List<RentItService.MediaReview> mediaReviews = new List<MediaReview>();
